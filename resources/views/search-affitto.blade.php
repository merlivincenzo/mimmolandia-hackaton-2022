...
@section('content')
    <div class="card">
        <form action="" method="get">
            ...
            <a href="{{ route('searchAffitto') }}">Reset filtri</a>
            <a href="{{ route('search') }}">Vai alle vendite</a>
        </form>
    </div>
    <div class="card">
        <div class="card-body">
            ...
            <table class="table table-responsive-sm table-striped table-bordered datatables" id="statsTable">
                ...
                <tbody>
                    @foreach(\App\Models\City::all() as $city)
                        @php
                            $avg = \App\Models\Apartment::where('city_id', $city->id)->where('contract', 'affitto')->avg('price') / \App\Models\Apartment::where('city_id', $city->id)->where('contract', 'affitto')->avg('square_mt');
                        @endphp
                        <tr>
                            <td>{{ $city->name }}</td>
...
@push('after_scripts')
    ...
    <script>
        ...
        @php
            $url = route("searchAffitto", ['distance' => request()->distance, 'city' => request()->city]);
            $urlAvg = route("searchAffittoAvg", ['distance' => request()->distance, 'city' => request()->city]);
        @endphp
